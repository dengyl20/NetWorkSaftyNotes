...
- 原理图：<img src=".\assets\image-20221121022810602.png" alt="image-20221121022810602" style="zoom:50%;" />
...
- <img src=".\assets\image-20221121025133853.png" alt="image-20221121025133853" style="zoom:50%;" />
...
- 基于表单的身份认证：改进1  
- <img src=".\assets\image-20221121031331256.png" alt="image-20221121031331256" style="zoom:33%;" />
...
- 无线网络分类：<img src=".\assets\image-20221121032929071.png" alt="image-20221121032929071" style="zoom:33%;" />
...
- 有线等效保密协议WEP：
  ...
  - <img src=".\assets\image-20221121034242779.png" alt="image-20221121034242779" style="zoom:33%;" />

  - <img src=".\assets\image-20221121034920909.png" alt="image-20221121034920909" style="zoom:33%;" />

  - <img src=".\assets\image-20221121034940521.png" alt="image-20221121034940521" style="zoom:33%;" />

  - <img src=".\assets\image-20221121035005807.png" alt="image-20221121035005807" style="zoom:33%;" />
...
- WPA1：因为WEP的严重安全漏洞， 2002年Wi-Fi联盟制定了WPA1，这
  ...
  - IEEE 802.1x  ：
    ...
    - 认证过程：<img src=".\assets\image-20221121040016385.png" alt="image-20221121040016385" style="zoom:33%;" />

- WEP-WPA1-WPA2比较：<img src=".\assets\image-20221121040115484.png" alt="image-20221121040115484" style="zoom:33%;" />  
...
- 安全目标、服务、机制的关系：<img src=".\assets\image-20221121040337369.png" alt="image-20221121040337369" style="zoom:33%;" />
...
- X.800提供了以下安全服务：<img src=".\assets\image-20221121041832524.png" alt="image-20221121041832524" style="zoom:33%;" />
...
<img src=".\assets\image-20221121043013564.png" alt="image-20221121043013564" style="zoom:33%;" />

### 特定的安全机制

<img src=".\assets\image-20221121043050985.png" alt="image-20221121043050985" style="zoom:50%;" />
...
- <img src=".\assets\image-20221121043637873.png" alt="image-20221121043637873" style="zoom:33%;" />
...
- IPsec文档：
  ...
  - <img src=".\assets\image-20221121052716286.png" alt="image-20221121052716286" style="zoom:50%;" />

### IPsec提供的服务

<img src=".\assets\image-20221121052844549.png" alt="image-20221121052844549" style="zoom:50%;" />
...
- 安全关联数据库SADB：包含了（SPI，IPDA，AH/ESP）三元组索引<img src=".\assets\image-20221121054558060.png" alt="image-20221121054558060" style="zoom:50%;" />
...
-   SPDB入口由下列选择子确定：![image-20221122155748296](./assets/image-20221122155748296.png)  
...
-   SADB与SPDB：
    ...
    <img src="./assets/image-20221122161648704.png" alt="image-20221122161648704" style="zoom:33%;" />
...
-   传输模式：主要为上层协议提供保护，同时增加了IP包载荷的保护  
    ...
    -   <img src="./assets/image-20221122161957784.png" alt="image-20221122161957784" style="zoom:33%;" />
...
-   隧道模式：对整个IP包提供保护  
    ...
    -   <img src="./assets/image-20221122162432713.png" alt="image-20221122162432713" style="zoom:33%;" />
...
-   在传输模式中使用AH：
    -   通常以端到端方式实现（在主机上实现）  
    -   不修改IP头，只添加AH头  
    -   <img src="./assets/image-20221122165657509.png" alt="image-20221122165657509" style="zoom:50%;" />
-   在隧道模式中使用AH：
    -   通常在防火墙或路由器上实现  
    -   把整个IP包作为数据，增加一个新的IP头、 AH头  
    -   <img src="./assets/image-20221122165743382.png" alt="image-20221122165743382" style="zoom:50%;" />
-   在传输模式和隧道模式中使用ESP：
...
-   在传输模式中使用ESP：
    ...
    -   <img src="./assets/image-20221122173103587.png" alt="image-20221122173103587" style="zoom:50%;" />
-   在隧道模式中使用ESP：
    -   隧道模式的ESP用于加密整个IP包， 可用于建设虚拟专用网  
    -   将ESP头作为包的前缀，并在包后附加ESP尾，然后对其进行加密  
    -   <img src="./assets/image-20221122173227726.png" alt="image-20221122173227726" style="zoom:50%;" />
...
-   <img src="./assets/image-20221122162706678.png" alt="image-20221122162706678" style="zoom:50%;" />

-   <img src="./assets/image-20221122162811029.png" alt="image-20221122162811029" style="zoom:33%;" />
...
-   反重放攻击：
    ...
    -   发送方不允许循环计数，否则，同一个序列号就可以产生多个合法的包；如果该序列号到达232-1，则SA必须中止，用新的密钥协商声称新的SA  

    -   由于IP协议是无连接的，IPsec认证文档中声明，接收方应该实现一个大小为W的窗口（W的默认值为64），如下图：
    -   <img src="./assets/image-20221122164624741.png" alt="image-20221122164624741" style="zoom:50%;" />  

-   完整性校验值：
    -   认证数据AD域包含完整性校验值ICV， ICV是一种报文认证编码MAC或者MAC算法生成的截断代码  
    -   <img src="./assets/image-20221122165213156.png" alt="image-20221122165213156" style="zoom:50%;" />

-   <img src="./assets/image-20221122170636750.png" alt="image-20221122170636750" style="zoom:33%;" />

### IPsec:封装安全载荷ESP

-   封装安全载荷ESP提供保密性服务包括报文内容保密和流量限制保密，ESP还可以提供和AH同样的认证服务 
-   ESP的格式：
    -   <img src="./assets/image-20221122171325907.png" alt="image-20221122171325907" style="zoom:33%;" /> 
    -   <img src="./assets/image-20221122171340645.png" alt="image-20221122171340645" style="zoom:33%;" />
...
-   ESP对接收数据包的处理：
    ...
    <img src="./assets/image-20221122173302336.png" alt="image-20221122173302336" style="zoom:33%;" />

-   <img src="./assets/image-20221122173321435.png" alt="image-20221122173321435" style="zoom:33%;" />
...
-   AH和ESP典型组合
    -   <img src="./assets/image-20221122180928752.png" alt="image-20221122180928752" style="zoom:33%;" />
...
-   IKE和IPsec：<img src="./assets/image-20221123165302427.png" alt="image-20221123165302427" style="zoom:33%;" />
...
-   ISAKMP报文头：
    ...
    -   <img src="./assets/image-20221123165612871.png" alt="image-20221123165612871" style="zoom:33%;" /> 

    -   <img src="./assets/image-20221123165858941.png" alt="image-20221123165858941" style="zoom:33%;" />

    -   <img src="./assets/image-20221123170058519.png" alt="image-20221123170058519" style="zoom:33%;" />

- IKE的载荷（13种）：ISAKMP定义了13种载荷，具有相同格式的载荷头  

  - ![image-20221123190752977](.\assets\image-20221123190752977.png)

  - ![](.\assets\image-20221123191934833.png)

  - ![image-20221123191956371](.\assets\image-20221123191956371.png)
...
- ![image-20221123193742768](.\assets\image-20221123193742768.png)
...
- 主模式概览：![image-20221123194224534](.\assets\image-20221123194224534.png)

- 主模式SA交换：

  - ![image-20221123202553362](.\assets\image-20221123202553362.png)

  - ![image-20221123202611086](.\assets\image-20221123202611086.png)

  - ![image-20221123203204521](.\assets\image-20221123203204521.png)
...
- 快速模式概览：
  ...
  - ![image-20221123203508443](.\assets\image-20221123203508443.png)

  - ![image-20221123203652170](.\assets\image-20221123203652170.png)

  - ![image-20221123203708238](.\assets\image-20221123203708238.png)

  - ![image-20221123203828668](.\assets\image-20221123203828668.png)
...
- ![image-20221123204516999](.\assets\image-20221123204516999.png)
...
![image-20221123205251353](.\assets\image-20221123205251353.png)
...
- SSL的使用案例：
  ...
  ![image-20221126045407497](.\assets\image-20221126045407497.png)
...
- 会话和连接的关系：<img src=".\assets\image-20221126050035950.png" alt="image-20221126050035950" style="zoom:50%;" />
...
- SSL记录协议的工作流程：
  ...
  ![image-20221126050826865](./assets/image-20221126050826865.png)

- SSL记录协议的操作流程：

  ![image-20221126050949886](./assets/image-20221126050949886.png)
...
- 握手协议消息类型：
  ...
  ![image-20221126052106452](./assets/image-20221126052106452.png)

- SSL握手协议阶段：

  ![image-20221126052844704](./assets/image-20221126052844704.png)

- 阶段1：建立安全能力

  ![image-20221126052958859](./assets/image-20221126052958859.png)

- 阶段2：服务器认证和密钥交换

  ![image-20221126053436363](./assets/image-20221126053436363.png)

- 阶段3：客户认证和密钥交换

  ![image-20221126053903704](./assets/image-20221126053903704.png)

![image-20221126053926953](./assets/image-20221126053926953.png)

- 阶段4：结束

  ![image-20221126054025258](./assets/image-20221126054025258.png)

### SSL告警协议和SSL修改密码规约协议

![image-20221126054217565](./assets/image-20221126054217565.png)

![image-20221126054227499](./assets/image-20221126054227499.png)

- SSL安全性分析：

  ![image-20221126054401931](./assets/image-20221126054401931.png)
...
- Web流量安全方法：
  ...
  ![image-20221126061059334](./assets/image-20221126061059334.png)
...
- ![image-20221126061442781](./assets/image-20221126061442781.png)
...
![image-20221126062041643](./assets/image-20221126062041643.png)
...
- ![image-20221126062731332](./assets/image-20221126062731332.png)

- ![image-20221126062821508](./assets/image-20221126062821508.png)

- ![image-20221126062935982](./assets/image-20221126062935982.png)
...
- ![image-20221126063137355](./assets/image-20221126063137355.png)

### 支付处理流程Step3：支付获取

![image-20221126063208147](./assets/image-20221126063208147.png)
